...
def tmrt_iso7726(Tg, Ta, va, diameter=.04, emissivity=.95):
    Tg=np.asarray(Tg,float); Ta=np.asarray(Ta,float); va=np.maximum(np.asarray(va,float),1e-3)
...
def compute_utci(ta, tmrt, rh, v10):
    ...
    out=[]
    for a,m,h,v in zip(np.asarray(ta),np.asarray(tmrt),np.asarray(rh),np.asarray(v10)):
        if not np.all(np.isfinite([a,m,h,v])): out.append(np.nan); continue
        value=utci(tdb=float(a),tr=float(m),v=float(np.clip(v,.5,17)),rh=float(h),limit_inputs=True)
