...
def extract_candidates(video_path: str | Path,
                       output_dir: str | Path,
                       candidate_fps: float = 0.5,
...
def greedy_diverse(frames: list[Path], k: int) -> list[Path]:
    ...
    while len(selected) < k:
        ...
        for f in frames:
            ...
            d = min(
                cv2.compareHist(hists[f], hists[s], cv2.HISTCMP_BHATTACHARYYA)
                for s in selected)
...
def deduplicate(frames: list[Path], threshold: float = 0.15) -> list[Path]:
    ...
    for f in frames:
        if not any(
                cv2.compareHist(hists[f], hists[g], cv2.HISTCMP_BHATTACHARYYA)
                < threshold for g in final):
            final.append(f)
    return final


def copy_selected(frames: list[Path],
                  output_dir: str | Path,
                  clear=True) -> list[Path]:
