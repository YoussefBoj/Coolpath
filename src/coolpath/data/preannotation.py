...
def segment_equirect_cubemap(model,
                             equi_bgr: np.ndarray,
                             face_width: int = 768) -> np.ndarray:
    import py360convert
    ...
    equi = py360convert.c2e(pred_faces,
                            h=h,
                            w=w,
                            mode='nearest',
                            cube_format='dict')
...
def export_cvat_polygons(model,
                         image_paths: list[Path],
                         xml_path: str | Path,
                         preview_dir: str | Path | None = None,
                         face_width: int = 768,
                         min_contour_area: int = 600) -> Path:
    root = ET.Element('annotations')
    ET.SubElement(root, 'version').text = '1.1'
    labels_el = ET.SubElement(
    ...
    for name, color in zip(CITYSCAPES_CLASSES, CITYSCAPES_PALETTE):
        ...
        ET.SubElement(le, 'name').text = name
    ...
    for image_id, path in enumerate(image_paths):
        ...
        if preview_dir:
            ...
            for cid in np.unique(mask):
                if cid < len(CITYSCAPES_CLASSES):
                    color_mask[mask == cid] = CITYSCAPES_PALETTE[cid][::-1]
            cv2.imwrite(str(preview_dir / path.name),
                        cv2.addWeighted(orig, .6, color_mask, .4, 0))
        image_el = ET.SubElement(root,
                                 'image',
                                 id=str(image_id),
                                 name=path.name,
                                 width=str(w),
        ...
        for cid in np.unique(mask):
            ...
            contours, _ = cv2.findContours((mask == cid).astype(np.uint8),
                                           cv2.RETR_EXTERNAL,
                                           cv2.CHAIN_APPROX_SIMPLE)
            for cnt in contours:
                if cv2.contourArea(cnt) < min_contour_area: continue
                approx = cv2.approxPolyDP(cnt,
                                          0.002 * cv2.arcLength(cnt, True),
                                          True)
                if len(approx) < 3: continue
                pts = ';'.join(f'{p[0][0]:.1f},{p[0][1]:.1f}' for p in approx)
                ET.SubElement(image_el,
                              'polygon',
                              label=CITYSCAPES_CLASSES[int(cid)],
                              source='auto',
                              occluded='0',
                              points=pts,
                              z_order='0')
    ...
    xml_path.write_text(minidom.parseString(
