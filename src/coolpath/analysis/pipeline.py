...
def timestamp_images(
    ...
    for name in ind.image.astype(str):
        ...
        timestamps.append(
            start +
            pd.to_timedelta(frame / fps.get(chapter, default_fps), unit="s"))
    ind["ts"] = pd.to_datetime(timestamps)
    return ind


def retain_frames_with_sensor_support(ind: pd.DataFrame,
                                      cp: pd.DataFrame,
                                      tolerance_s=2.0):
    ...
    out = out[out["_sensor"].notna()].drop(columns="_sensor").reset_index(
        drop=True)
...
def aggregate(cp, ind, window_s, min_sensor=2, min_images=1):
    ...
    sensor_cols = [
        c for c in [
            "Kdown", "Tg", "Ta", "RH", "WS", "v10", "Tmrt", "UTCI", "Prt_Mid",
            "Kdown_hist"
        ] if c in cp
    ]
    indicator_cols = [
        c for c in ind.columns if c not in {"image", "ts", "bin"}
        and pd.api.types.is_numeric_dtype(ind[c])
    ]
    ...
    table = table[(table.n_mesures >= min_sensor)
                  & (table.n_images >= min_images)].copy()
    ...
    for base in bases:
        ...
        for target in targets:
            if target not in table:
                continue
            common = (np.isfinite(table[base])
                      & np.isfinite(table[historical])
                      & np.isfinite(table[target]))
            current = robust_correlation(table.loc[common, base],
                                         table.loc[common, target])
            hist = robust_correlation(table.loc[common, historical],
                                      table.loc[common, target])
            rows.append({
                "indicateur": base,
                "cible": target,
...
def run_analysis(
    ...
    ind, orphan_count = retain_frames_with_sensor_support(
        ind, cp, sensor_tolerance_s)
    ...
    best, grid = calibrate_globe_inertia(cp.Kdown.to_numpy(), cp.Tg.to_numpy(),
                                         dt)
    pd.DataFrame(grid).to_csv(output_dir / "calibration_inertie_globe.csv",
                              index=False)
    ...
    for window_s, table in tables.items():
        table.to_csv(output_dir / f"table_segments_{window_s}s.csv",
                     index=False)

    ref = tables[int(reference_window)]
    targets = [
        c for c in [
            "Kdown", "Kdown_hist", "Tg", "Ta", "RH", "v10", "Tmrt", "UTCI",
            "Prt_Mid"
        ] if c in ref
    ]
    predictors = current_indicators + [
        f"{c}_hist" for c in historized if f"{c}_hist" in ref
    ]
    ...
    matrix["rejet_fdr"], matrix["q_value"] = fdr_bh(
        matrix.p_corrigee.to_numpy())
    matrix.to_csv(output_dir / "matrice_correlations.csv", index=False)

    comparison_targets = [
        c
        for c in ["Kdown", "Tg", "Ta", "RH", "v10", "Tmrt", "UTCI", "Prt_Mid"]
        if c in ref
    ]
    comparison = _build_current_history_comparison(ref, historized,
                                                   comparison_targets)
    comparison.to_csv(output_dir / "comparaison_courant_vs_historique.csv",
                      index=False)
            ...
            for target in [c for c in ["Tmrt", "UTCI"] if c in ref]:
                ...
                for label, control_names in control_sets:
                    ...
                    rho, p, n = partial_spearman_block_permutation(
                        ...
                        n_perm=999,
                        block_size=max(2,
                                       int(round(tau / reference_window)) + 1),
                    )
                    partial.append({
                        "indicateur": predictor,
                        "cible": target,
    ...
    pd.DataFrame(partial).to_csv(output_dir / "correlations_partielles.csv",
                                 index=False)
    ...
    pd.DataFrame(moran).to_csv(output_dir / "autocorrelation_moran.csv",
                               index=False)

    # Reduced HAC models: A (morphology history), B (+ radiation history), A' (current morphology).
    target = "Tmrt" if "Tmrt" in ref and ref.Tmrt.notna().sum(
    ) > 20 else "UTCI"
    model_rows = []
    models = {
        "A_hist": ["GVI_haute_pct_hist", "SVF_hist", "t_mid_s"],
        "B_hist_Kdown":
        ["GVI_haute_pct_hist", "SVF_hist", "t_mid_s", "Kdown_hist"],
    ...
    for model_label, names in models.items():
        ...
        for name, beta, se, p in zip(fit["names"], fit["beta"], fit["se_hac"],
                                     fit["p_hac"]):
            model_rows.append({
                "modele": model_label,
                "cible": target,
    ...
    pd.DataFrame(model_rows).to_csv(output_dir / "modeles_reduits.csv",
                                    index=False)
    ...
    pd.DataFrame([summary]).to_csv(output_dir / "resume_analyse.csv",
                                   index=False)
    return summary
