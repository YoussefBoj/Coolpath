...
def load_mmseg_model(config_path: str | Path, checkpoint_path: str | Path, device='cuda:0'):
...
def run_inference(model, images_dir: str | Path, output_dir: str | Path, overwrite=False):
    out=ensure_dir(output_dir); rows=[]
    for i,p in enumerate(list_images(images_dir),1):
        dst=out/f'{p.stem}_sem.png'
        if dst.exists() and not overwrite:
            rows.append(dst); continue
        mask=predict_semantic(model,str(p)); cv2.imwrite(str(dst),mask); rows.append(dst)
