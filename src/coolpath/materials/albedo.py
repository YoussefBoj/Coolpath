...
BASE_ALBEDO = {
    ...
    B['Glass']: .12,
    B['Metal']: .38,
    B['Paint/Coating/Plaster']: .45,
    B['Foliage']: .22,
    B['Tree']: .22,
}
ALBEDO_OVERRIDES = {
    (SEM_VEGETATION, B['Foliage']): .20,
    (SEM_VEGETATION, B['Tree']): .20,
    (SEM_TERRAIN, B['Foliage']): .21,
    (SEM_TERRAIN, B['Tree']): .21,
    (SEM_TERRAIN, B['Soil/Mud']): .17,
    (SEM_TERRAIN, B['Sand']): .40
}
...
def build_albedo_lut(paint_albedo: float | None = None) -> np.ndarray:
    ...
    for mat, val in BASE_ALBEDO.items():
        lut[:, mat] = val
    for key, val in ALBEDO_OVERRIDES.items():
        lut[key] = val
    if paint_albedo is not None:
        lut[:, B['Paint/Coating/Plaster']] = paint_albedo
    return lut
...
def shadow_correct_rgb(rgb_pixels: np.ndarray,
    ...
    scale = np.where(lum > 1.0, ref / np.maximum(lum, 1e-6), 1.0)
