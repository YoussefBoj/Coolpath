...
def compute_gvi(mask: np.ndarray, valid_mask: np.ndarray | None=None) -> tuple[float,float]:
    ...
    h,w=mask.shape; weights=np.sin((np.arange(h)+0.5)/h*np.pi)[:,None]
    weighted=float((mask*valid_mask*weights).sum()/max((valid_mask*weights).sum(),1e-12)*100)
    return raw,weighted
