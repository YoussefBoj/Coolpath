...
def band(h,w,lo,hi):
    theta,_,_=svf_row_weights(h); m=np.zeros((h,w),bool); m[(theta>=np.radians(lo))&(theta<=np.radians(hi))]=True; return m
...
def test_solid_angle_gvi_differs_near_pole():
    h,w=180,360; m=np.zeros((h,w),bool); m[:20]=True; raw,omega=compute_gvi(m,np.ones_like(m,bool)); assert omega<raw
